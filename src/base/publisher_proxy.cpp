...
namespace huleibao {
PublisherProxy::PublisherProxy(std::shared_ptr<StubWrapper> core_stub)
    : m_core_stub_(core_stub) {}

void PublisherProxy::AdvertiseTopic(std::string node_name,
  ...
  // - The actual RPC.
  Status status =
      m_core_stub_->GetStub()->AdvertiseTopic(&context, request, &reply);
  // - Act upon its status.
  if (!status.ok())
    LOG_FATAL(" - AdvertiseTopic " << status.error_code() << ": "
...
void PublisherProxy::Publish(std::string &topic_name,
  ...
  // - Send to Core
  Status status =
      m_core_stub_->GetStub()->PublishTopic(&context, topicData, &reply);
  // - Act upon its status.
  if (!status.ok())
    LOG_FATAL("AdvertiseTopic " << status.error_code() << ": "
                                << status.error_message());
