...
Status CoreServiceImpl::NodeGreet(ServerContext *context,
                                  const GreetData *request,
                                  CommonReply *reply) {
  // - parse input
...
Status CoreServiceImpl::NodeDisconnect(ServerContext *context,
                                       const GreetData *request,
                                       CommonReply *reply) {
  // - parse input
  const std::string &nodeName = request->node_name();
...
Status CoreServiceImpl::AdvertiseTopic(ServerContext *context,
  ...
  LOG_VERBOSE("[core] AdvertiseTopic from " << publisherName
                                            << ", topicName= " << topicName
  ...
  if (0 == m_topic_mapper_.count(topicName)) {
    ...
    std::string info =
        std::string("Topic ") + topicName + " already registered.";
    reply->set_info(info);
    reply->set_code(ReplyStatusCode::success);
  }
  return Status::OK;
...
Status CoreServiceImpl::SubscribeTopic(ServerContext *context,
  ...
  if (false == topic->AddSubscriber(subscriberName)) {
    LOG_WARN("[core] SubscribeTopic from " << subscriberName
                                           << ", topicName= " << topicName
                                           << ", but duplicated node name.");
  }

  LOG_VERBOSE("[core] SubscribeTopic from " << subscriberName
                                            << ", topicName= " << topicName);

  LOG_INFO("[core] Establish a connection with "
           << subscriberName << ", topicName= " << topicName);
  // - Wait for topic notification until the connection is disconnected
  while (!m_kill_subscribe_stream_of_[subscriberName] ||
         !context->IsCancelled()) {
    std::unique_lock<std::mutex> lock(topic->m_msg_mtx_);
    topic->m_input_condition_.wait(
        lock, [this, context, topic, subscriberName] {
    ...
    if (m_kill_subscribe_stream_of_[subscriberName] || context->IsCancelled())
      break;
    Topic::MessageType msgs =
        std::move(topic->GetLastestMessage(subscriberName));
    lock.unlock();
...
Status CoreServiceImpl::PublishTopic(ServerContext *context,
                                     const TopicData *request,
                                     CommonReply *reply) {
  // - parse input
  const std::string &topicName = request->topic_name();
...
/// publish request of topics
Status CoreServiceImpl::SetParam(ServerContext *context,
                                 const Parameter *request, CommonReply *reply) {
  // - parse input
...
/// publish request of topics
Status CoreServiceImpl::GetParam(ServerContext *context,
                                 const Parameter *request, CommonReply *reply) {
  // - parse input
  const std::string &paramName = request->param_name();
