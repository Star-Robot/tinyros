...
public:
  ...
  typename Publisher<MessageType>::Ptr Advertise(std::string topic_name,
                                                 int buffer_size) {
  ...
  template <typename MessageType>
  typename Subscriber<MessageType>::Ptr
  Subscribe(std::string topic_name, int buffer_size,
  ...
  typename Subscriber<MessageType>::Ptr
  Subscribe(std::string topic_name, int buffer_size,
            void (T::*callback_func)(typename MessageType::ConstPtr), T *obj) {
  ...
  /// \Note: ParamType=[int,long,u-long,longlong,float,double,map<k,v>]
  template <typename ParamType>
  ParamType GetParam(const std::string &param_name,
                     ParamType default_val = ParamType()) {
    // - seralized val
    std::string paramVal =
        m_param_service_->GetParam(param_name, ParamToString(default_val));
