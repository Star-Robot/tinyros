...
public:
  ...
  Subscriber(std::shared_ptr<StubWrapper> core_stub, std::string node_name,
    ...
    m_proxy_.reset(
        new SubscriberProxy(core_stub, node_name, topic_name, buffer_size));

    m_proxy_->m_thread_continue_ = true;
    // - Subscribe to each Node topic and start a stream reading thread
    m_stream_reader_thread_ =
        std::thread(&SubscriberProxy::StreamReaderThread, m_proxy_.get());
  ...
  void TopicCallBackThread() {
    while (m_proxy_->m_thread_continue_) {
      ...
      m_proxy_->m_input_condition_.wait(lock, [this] {
        return !m_proxy_->m_thread_continue_ ||
               !m_proxy_->m_message_queue_.empty();
      });
      // - Jump out of the loop if the thread exits
      if (!m_proxy_->m_thread_continue_)
        break;

      SubscriberProxy::MessageType msgs =
          std::move(m_proxy_->m_message_queue_.front());
