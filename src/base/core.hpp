...
private:
  ...
  Status NodeGreet(ServerContext *context, const GreetData *request,
                   CommonReply *reply) override;
  /// The node will disconnect with the core
  Status NodeDisconnect(ServerContext *context, const GreetData *request,
                        CommonReply *reply) override;
  /// Register a message queue with a size of buffer_size,
  /// which is used to store the topic topic_name of the node publisher
  Status AdvertiseTopic(ServerContext *context, const AdvertiseRequest *request,
                        CommonReply *reply) override;
  /// Subscribers request notification of topics
  Status SubscribeTopic(ServerContext *context, const SubscribeRequest *request,
                        ServerWriter<TopicData> *writer) override;
  /// publish request of topics
  Status PublishTopic(ServerContext *context, const TopicData *request,
                      CommonReply *reply) override;
  /// set global parameter
  Status SetParam(ServerContext *context, const Parameter *request,
                  CommonReply *reply) override;
  /// get global parameter
  Status GetParam(ServerContext *context, const Parameter *request,
                  CommonReply *reply) override;
